...
let pageStart = lines.findIndex(l => l.includes('@page {'));
if (pageStart !== -1) {
  let pageEnd = pageStart;
  while (!lines[pageEnd].includes('}')) {
    pageEnd++;
  }
  lines.splice(pageStart, pageEnd - pageStart + 1, 
`              @page {
...
let printStart = lines.findIndex(l => l.includes('@media print {'));
if (printStart !== -1) {
  lines.splice(printStart + 1, 0, 
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('className="preview-page-padding relative flex-1 flex flex-col transition-all duration-300') && lines[i].includes('<div')) {
    openings.push(i);
  }
}

...
import { curriculumData, sidebarSubjects } from './guide-data';
import Image from 'next/image';

export default function GuideDetailsPage({ params }: { params: { id: string } }) {
  // Mock data fetching
  const currentSubject = sidebarSubjects.find(s => s.id === params.id) || sidebarSubjects[0];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 w-full flex flex-col bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="px-6 py-4 flex flex-wrap items-center gap-2 border-b border-slate-200 dark:border-slate-800">
            <span className="px-3 py-1 bg-[#107c41] text-white text-[12px] font-bold rounded-full">
              MCQ: 2.3k
            </span>
            <span className="px-3 py-1 bg-[#107c41] text-white text-[12px] font-bold rounded-full">
              CQ: 1.8k
            </span>
            <span className="px-3 py-1 bg-[#0b5c30] text-white text-[12px] font-bold rounded-full">
              Board Exam: 1
            </span>
        ...
        <div className="w-full lg:w-[340px] shrink-0">
          <GuideSidebar subjects={sidebarSubjects} activeId={params.id} />
        </div>

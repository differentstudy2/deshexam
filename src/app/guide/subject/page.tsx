...
import { getGuideSubjects, getGuideTextbooksBySubject } from '@/lib/firebase/guide';
import { ChevronRight, BookOpen, Layers } from 'lucide-react';

export const metadata = {
  title: 'Subjects - Academy Guide',
  description: 'Select your subject to view textbooks and curriculum.'
};
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020817] text-slate-800 dark:text-slate-200 font-sans pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div className="hidden sm:flex items-center text-[13px] text-slate-500 dark:text-slate-400 font-medium border-l border-slate-200 dark:border-slate-800 pl-6">
              ...
              <ChevronRight className="w-3.5 h-3.5 mx-2" />
              <span className="text-slate-800 dark:text-slate-200">Subjects</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
          ...
          <p className="text-slate-500">Select your subject to get started with the curriculum.</p>
        </div>
...
async function SubjectCard({ subject }: { subject: any }) {
  const textbooks = await getGuideTextbooksBySubject(subject.id);
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col transition-shadow hover:shadow-md">
      <Link href={`/guide/subject/${subject.slug || subject.id}`} className="block">
      ...
      <div className="p-5 flex-1">
        {textbooks.length === 0 ? (
          <p className="text-sm text-slate-400 italic py-2">No textbooks added yet.</p>
        ) : (
          <ul className="space-y-2">
            {textbooks.slice(0, 5).map((tb: any) => (
              <li key={tb.id}>
                <Link 
                  href={`/guide/textbook/${tb.slug || tb.id}`} 
            ...
            {textbooks.length > 5 && (
              <li className="pt-2">
                ...
                >
                  View all {textbooks.length} textbooks
                </Link>
              </li>
            )}
          </ul>

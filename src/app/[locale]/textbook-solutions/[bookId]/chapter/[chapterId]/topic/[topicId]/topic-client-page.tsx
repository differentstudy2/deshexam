...
export default function TopicClientPage() {
    ...
    const [topicExams, setTopicExams] = useState<Exam[] | null>(null);
    ...
    const handleTabChange = async (tab: string) => {
        const fetchContent = async (type: "Exam" | "Mock Test" | "Quiz", setter: React.Dispatch<React.SetStateAction<Exam[] | null>>) => {
            ...
            try {
                ...
                setter(allContent.filter((item: any) => item.topicId === topicId));
            } catch (e) {
        ...
        if (tab === 'exams' && topicExams === null) {
            fetchContent('Exam', setTopicExams);
    ...
    const filteredResources = useMemo(() => {
        if (!activeTopic?.resources) return [];
        if (!resourceFilter) return activeTopic.resources;
        return activeTopic.resources.filter(res => 
            res.title.toLowerCase().includes(resourceFilter.toLowerCase())
        );
    }, [activeTopic, resourceFilter]);
    ...
    const PracticeSetItem = ({ practiceSet }: { practiceSet: PracticeSet }) => {
            ...
            <div className="p-4 border rounded-lg hover:bg-accent flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
                  <Button size="sm" asChild className="flex-1">
                      <Link href={`/textbook-solutions/practice-set/${practiceSet.id}/textbook/${textbookId}/chapter/${chapterId}/topic/${topicId}`}>Start Practice</Link>
                  </Button>
                  <Button 
                      size="sm" 
                      variant="outline" 
                      className="flex-1"
                      onClick={() => handleDownloadPdf(practiceSet)}
    ...
    const ContentList = ({ items, type }: { items: Exam[] | null, type: string }) => {
        ...
        if(items.length === 0) {
            return <p className="text-muted-foreground text-center py-8">No {type.toLowerCase()}s available for this topic.</p>
        }
        return (
                        ...
                        <div>
                             ...
                             <Tabs defaultValue="content" className="w-full mt-8" onValueChange={handleTabChange}>
                                <TabsList className="grid w-full grid-cols-2 md:grid-cols-6 h-auto">
                                    ...
                                    {activeTopic?.resources && activeTopic.resources.length > 0 && <TabsTrigger value="resources">Resources</TabsTrigger>}
                                    {activeTopic?.practiceSets && activeTopic.practiceSets.length > 0 && <TabsTrigger value="practice-sets">Practice Sets</TabsTrigger>}
                                ...
                                <TabsContent value="content" className="mt-6">
                                     {activeTopic?.content ? (
                                        ...
                                        <div className="text-center text-muted-foreground py-16">
                                            ...
                                            <p>There is no written content available for this topic yet. Check out the other tabs for resources or practice sets!</p>
                                        </div>
                                     )}
                                </TabsContent>
                                ...
                                <TabsContent value="practice-sets" className="mt-6">
                                    {activeTopic?.practiceSets && activeTopic.practiceSets.length > 0 ? (
                                        <div className="space-y-4">
                                            {activeTopic.practiceSets.map(ps => <PracticeSetItem key={ps.id} practiceSet={ps} />)}
                                        </div>
                                ...
                                <TabsContent value="exams" className="mt-6">
                                    <ContentList items={topicExams} type="Exam" />
                                </TabsContent>
                            </Tabs>

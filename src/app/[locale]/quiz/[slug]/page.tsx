...
export default async function QuizLandingPage({ params }: Props) {
  ...
  const jsonLdBreadcrumb = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": test.title,
        "item": `https://deshexam.com/quiz/${test.slug}`
      }
    ]
  };
  ...
  const jsonLdCourse = {
    ...
    "@type": "Course",
    "name": test.title,
    "description": test.description || `quiz for ${test.title}`,
    "provider": {
      "@type": "Organization",
      "name": "DeshExam",
  ...
  const jsonLdProduct = {
    ...
    "offers": {
      ...
      "url": `https://deshexam.com/quiz/${test.slug}`,
      "shippingDetails": {
        ...
        <div className="relative bg-slate-950 overflow-hidden border-b border-white/5">
          ...
          <div className="relative container max-w-6xl mx-auto px-4 pt-6 pb-0">
            ...
            <nav className="flex items-center gap-1.5 text-xs text-slate-400 mb-8">
              ...
              <Link href="/quiz" className="hover:text-white transition-colors">Quizzes</Link>
              <ChevronRight className="w-3 h-3" />
                ...
                <div className={cn(
                  ...
                  <div className={cn(
                    ...
                    (test.accessType === 'subscription' || test.accessType === 'both' || test.accessType === 'one_time')
                      ...
                      : "bg-gradient-to-r from-purple-600 to-fuchsia-600 border-purple-700/50"
                  )}>
                    <p className={cn(
                      ...
                      <Brain className={cn("w-4 h-4", (test.accessType === 'subscription' || test.accessType === 'both' || test.accessType === 'one_time') ? "text-amber-100 fill-amber-100" : "text-purple-300 fill-purple-300")} />
                      Quick Summary
                    </p>
                    {(test.accessType === 'subscription' || test.accessType === 'both' || test.accessType === 'one_time') && (
          ...
          {related.length > 0 && (
            <section className="mt-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                {related.map(r => (
                  <AssessmentCard key={r.id} assessment={r as any} type="Quiz" href={`/quiz/${r.slug}`} taxonomyString={r.taxonomyString} classNameString={r.classString} />
                ))}
              </div>
            </section>
          )}

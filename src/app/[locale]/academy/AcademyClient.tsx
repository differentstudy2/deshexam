...
export default function AcademyClient({ 
  ...
  preSelectedBoardSlug
}: { 
  preSelectedClassId?: string;
  preSelectedClassSlug?: string;
  preSelectedBoardSlug?: string;
}) {
  ...
  useEffect(() => {
    ...
    const fetchClasses = async () => {
      ...
      try {
        ...
        } else if (preSelectedClassSlug) {
          const matchedClass = fetchedClasses.find(c => c.slug === preSelectedClassSlug || c.fullSlug?.includes(preSelectedClassSlug));
          if (matchedClass) {
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-[#020817] text-slate-800 dark:text-slate-200">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {loading ? (
          ...
          <>
            {/* Hero Section */}
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-600 via-teal-600 to-emerald-900 dark:from-slate-900 dark:via-emerald-950/40 dark:to-slate-900 text-white shadow-xl mb-10 border border-emerald-500/30 dark:border-emerald-800/30">
              {/* Background Decorative Elements */}
              <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-white/10 dark:from-emerald-500/5 via-transparent to-transparent opacity-60"></div>
              <div className="absolute -top-32 -right-32 w-[30rem] h-[30rem] bg-teal-400/20 dark:bg-teal-500/10 rounded-full blur-3xl"></div>
              <div className="absolute -bottom-32 -left-32 w-[25rem] h-[25rem] bg-emerald-400/20 dark:bg-emerald-500/10 rounded-full blur-3xl"></div>
              
              <div className="relative p-6 md:p-8 lg:p-10 flex flex-col lg:flex-row items-center justify-between gap-6">
                <div className="flex-1 w-full lg:max-w-3xl xl:max-w-4xl space-y-4">
                  {/* Greeting & Badge Row */}
                  <div className="flex flex-wrap items-center gap-3">
                    {userProfile?.displayName ? (
                      <div className="text-emerald-100 font-semibold text-base border-r border-white/20 dark:border-emerald-800/50 pr-3">
                        Welcome back, {userProfile.displayName}!
                      </div>
                    ) : null}
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 dark:bg-white/5 border border-white/20 dark:border-white/10 text-xs font-medium backdrop-blur-md shadow-inner">
                      <span className="relative flex h-2 w-2">
                        ...
                        <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-400 dark:bg-emerald-500"></span>
                      </span>
                  ...
                  <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight">
                    {selectedClassTitle} Academy <br/>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 to-teal-100 dark:from-emerald-400 dark:to-teal-200">Textbooks & Guides</span>
                  </h1>
                  
                  <p className="text-emerald-50/90 dark:text-emerald-100/80 text-base max-w-xl leading-relaxed font-medium">
                      ...
                      {classesList.map((cls) => {
                        ...
                        return (
                          <Link 
                            ...
                            className={cn(
                              "px-4 py-1.5 text-xs font-bold rounded-full shadow-sm transition-colors cursor-pointer border inline-flex",
                              selectedClassId === cls.id 
                                ? "bg-white text-emerald-800 border-white dark:bg-emerald-500 dark:text-white dark:border-emerald-500" 
                  ...
                  <div className="flex flex-wrap lg:flex-nowrap gap-2 pt-2">
                    <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 backdrop-blur-md rounded-xl px-3 py-2 border border-white/10 dark:border-white/5 shadow-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors">
                      <div className="bg-emerald-400/20 dark:bg-emerald-500/20 p-1.5 rounded-lg border border-white/10 dark:border-white/5 shrink-0">
                        <BookOpen className="w-4 h-4 text-emerald-100 dark:text-emerald-200" />
                      </div>
                      <div className="whitespace-nowrap">
                        <div className="text-xl font-black">{filteredSubjectsData.length}</div>
                        <div className="text-[9px] text-emerald-100/80 dark:text-emerald-200/70 uppercase tracking-wider font-bold">Textbooks</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 backdrop-blur-md rounded-xl px-3 py-2 border border-white/10 dark:border-white/5 shadow-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors">
                      <div className="bg-blue-400/20 dark:bg-blue-500/20 p-1.5 rounded-lg border border-white/10 dark:border-white/5 shrink-0">
                        <Target className="w-4 h-4 text-blue-100 dark:text-blue-200" />
                      </div>
                      <div className="whitespace-nowrap">
                        <div className="text-xl font-black">1.5k+</div>
                        <div className="text-[9px] text-blue-100/80 dark:text-blue-200/70 uppercase tracking-wider font-bold">Questions</div>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 backdrop-blur-md rounded-xl px-3 py-2 border border-white/10 dark:border-white/5 shadow-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors">
                      <div className="bg-purple-400/20 dark:bg-purple-500/20 p-1.5 rounded-lg border border-white/10 dark:border-white/5 shrink-0">
                        <Activity className="w-4 h-4 text-purple-100 dark:text-purple-200" />
                      </div>
                      <div className="whitespace-nowrap">
                        <div className="text-xl font-black">50+</div>
                        <div className="text-[9px] text-purple-100/80 dark:text-purple-200/70 uppercase tracking-wider font-bold">Mock Tests</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 backdrop-blur-md rounded-xl px-3 py-2 border border-white/10 dark:border-white/5 shadow-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors">
                      <div className="bg-amber-400/20 dark:bg-amber-500/20 p-1.5 rounded-lg border border-white/10 dark:border-white/5 shrink-0">
                        <ListChecks className="w-4 h-4 text-amber-100 dark:text-amber-200" />
                      </div>
                      <div className="whitespace-nowrap">
                        <div className="text-xl font-black">800+</div>
                        <div className="text-[9px] text-amber-100/80 dark:text-amber-200/70 uppercase tracking-wider font-bold">MCQ</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 backdrop-blur-md rounded-xl px-3 py-2 border border-white/10 dark:border-white/5 shadow-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors">
                      <div className="bg-rose-400/20 dark:bg-rose-500/20 p-1.5 rounded-lg border border-white/10 dark:border-white/5 shrink-0">
                        <HelpCircle className="w-4 h-4 text-rose-100 dark:text-rose-200" />
                      </div>
                      <div className="whitespace-nowrap">
                        <div className="text-xl font-black">400+</div>
                        <div className="text-[9px] text-rose-100/80 dark:text-rose-200/70 uppercase tracking-wider font-bold">SAQ</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 backdrop-blur-md rounded-xl px-3 py-2 border border-white/10 dark:border-white/5 shadow-lg hover:bg-white/20 dark:hover:bg-white/10 transition-colors">
                      <div className="bg-indigo-400/20 dark:bg-indigo-500/20 p-1.5 rounded-lg border border-white/10 dark:border-white/5 shrink-0">
                        <FileText className="w-4 h-4 text-indigo-100 dark:text-indigo-200" />
                      </div>
                      <div className="whitespace-nowrap">
                        <div className="text-xl font-black">300+</div>
                        <div className="text-[9px] text-indigo-100/80 dark:text-indigo-200/70 uppercase tracking-wider font-bold">LAQ</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 pt-1">
                    <button className="flex items-center gap-2 bg-white dark:bg-emerald-500 text-emerald-900 dark:text-white px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-emerald-50 dark:hover:bg-emerald-600 transition-colors shadow-lg group">
                      Resume Learning
                      <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </button>
                    
                    <div className="flex flex-col w-full max-w-[200px] space-y-2">
                      <div className="flex justify-between text-xs font-semibold text-emerald-100 dark:text-emerald-200/80">
                ...
                <div className="hidden lg:flex relative shrink-0 items-center justify-center">
                  <div className="w-56 h-56 relative">
                    <div className="absolute inset-0 bg-gradient-to-tr from-emerald-400/30 dark:from-emerald-600/20 to-teal-300/30 dark:to-teal-600/20 rounded-full blur-2xl animate-pulse"></div>
                    <div className="relative w-full h-full bg-white/10 dark:bg-white/5 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-full flex items-center justify-center shadow-2xl">
                      <GraduationCap aria-label={`DeshExam Academy learning dashboard for ${selectedClassTitle} students`} className="w-24 h-24 text-emerald-50 dark:text-emerald-100 drop-shadow-xl" />
                      <span className="sr-only">DeshExam Academy learning dashboard for {selectedClassTitle} students</span>
                    </div>
                    
                    {/* Floating elements */}
                    <div className="absolute top-2 -left-2 bg-white/10 dark:bg-white/5 backdrop-blur-xl p-3 rounded-xl border border-white/20 dark:border-white/10 shadow-2xl animate-[bounce_3s_ease-in-out_infinite]">
                      <BookMarked className="w-6 h-6 text-emerald-200 dark:text-emerald-300" />
                    </div>
                    <div className="absolute bottom-6 -right-2 bg-white/10 dark:bg-white/5 backdrop-blur-xl p-3 rounded-xl border border-white/20 dark:border-white/10 shadow-2xl animate-[bounce_4s_ease-in-out_infinite]" style={{ animationDelay: '1s' }}>
                      <Lightbulb className="w-6 h-6 text-yellow-200 dark:text-yellow-300" />
                    </div>
                  </div>
                </div>
              </div>

...
export default function ManageTopicMockTestsPage() {
    ...
    const handleOpenDialog = (test: MockTest | null) => {
        ...
        const sourceArray = (test?.questionSource && Array.isArray(test.questionSource) ? test.questionSource : ['Random from Topic']) as ('Random from Chapter' | 'Random from Topic' | 'Textbook Exercise' | 'Solved Examples' | 'Previous Year Questions')[];
        
        const subtitle = test ? test.subtitle || `Mock Test ${tests.findIndex(t => t.id === test.id) + 1}` : `Mock Test ${tests.length + 1}`;
        setTestData(test ? { title: test.title, subtitle, difficulty: difficultyArray, questionSource: sourceArray } : { title: '', subtitle, difficulty: ['Medium'], questionSource: ['Random from Topic'] });
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Mock tests associated with this topic.
                    </p>
                </div>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow key={test.id}>
                                    ...
                                    <TableCell className="text-right space-x-2">
                                        <Button asChild variant="outline" size="sm">
                                            <Link href={`/textbook-solutions/mock-test/${test.id}/textbook/${textbookId}/chapter/${chapterId}/topic/${topicId}`}><Eye className="mr-2 h-4 w-4"/>View</Link>
                                        </Button>
                                        <Button asChild variant="outline" size="sm">
                                            <Link href={`/admin/textbooks/${textbookId}/chapter/${chapterId}/topic/${topicId}/mock-test/${test.id}`}><FileQuestion className="mr-2 h-4 w-4"/>Manage Questions</Link>
                                        </Button>
                                ...
                                <TableRow>
                                    ...
                                    No mock tests added to this topic yet.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                             ...
                             <div className="flex gap-2">
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem onSelect={() => generateTitle('[Topic Title] Mock Test')}>[Topic Title] Mock Test</DropdownMenuItem>
                                        <DropdownMenuItem onSelect={() => generateTitle('[Topic Title] Practice Test')}>[Topic Title] Practice Test</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </div>
                        </div>

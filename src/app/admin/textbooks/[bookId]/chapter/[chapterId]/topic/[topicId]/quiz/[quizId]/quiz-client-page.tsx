...
export default function QuizClientPage({ initialTest, initialTextbook, initialChapter, initialTopic }: { initialTest: Quiz, initialTextbook: Textbook, initialChapter: Chapter, initialTopic: Topic | null }) {
    ...
    const handleQuestionSubmit = async (data: QuestionFormValues) => {
        ...
        try {
            const contentRef = doc(db, "content", quizId);
            const contentDoc = await getDoc(contentRef);
            if (!contentDoc.exists()) throw new Error("Quiz not found");
            const currentQuestions = contentDoc.data().questions || [];

            if (editingQuestion) {
                const updatedQuestions = currentQuestions.map((q: any) => q.id === editingQuestion.id ? {...q, ...data} : q);
                await updateContent(quizId, { questions: updatedQuestions });
                setTest(prev => ({...prev, questions: updatedQuestions }));
                toast({ title: 'Question Updated' });
            } else {
                const newQuestion = { ...data, id: doc(collection(db, 'dummy')).id };

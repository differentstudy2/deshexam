...
function getUrlForPracticeSet(bookId: string, practiceSetId: string) {
    return `/${bookId}/text-practice-sets/${practiceSetId}`;
}
...
Difficulty
${itemData.difficulty?.join(', ') || ''}

Duration
${(itemData as any)?.durationMinutes || ''}

Questions
${(itemData as any)?.questions?.length || 0}

Total Marks
${(itemData as any)?.totalMarks || ''}

Pass Marks
${(itemData as any)?.passMarks || ''}

Negative Marking
${(itemData as any)?.negativeMarking ? 'Yes' : 'No'}
...
Price
${itemData.access === 'free' ? 'FREE' : 'Premium'}
...
Automatically display PRACTICE SET.
...
Automatically choose Start Practice.
                                        ...
                                        <Input id="ps-title" value={itemData.title} onChange={e => setItemData(p => ({...p, title: e.target.value}))} />
                                         <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                <DropdownMenuItem onSelect={() => generateTitle('[Subject] Full Syllabus Practice Set')}>[Subject] Full Syllabus Practice Set</DropdownMenuItem>
                                                <DropdownMenuItem onSelect={() => generateTitle('[Textbook Title] - Final Practice')}>[Textbook Title] - Final Practice</DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </div>
                                </div>

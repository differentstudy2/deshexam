            ...
            {node.type !== 'section' && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-slate-500 hover:bg-slate-100" onClick={(e) => e.stopPropagation()}>
                ...
                <DropdownMenuContent align="end" className="w-48" onClick={(e) => e.stopPropagation()}>
                  ...
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onSeoClick(node.id, node.type, node, () => {
                    if (refreshParent) refreshParent();
                  }); }}>
                    <Settings className="w-4 h-4 mr-2 text-amber-500" /> SEO Settings
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onEditClick(node.id, node.type, node.name, node.author, () => {
                    if (refreshParent) refreshParent();
                  }); }}>
                    <Edit2 className="w-4 h-4 mr-2 text-blue-500" /> Rename
                  </DropdownMenuItem>
                  {(node.type === 'chapter' || node.type === 'topic') && (
                    <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onMoveClick(node.id, node.type, node.name, () => {
                      if (refreshParent) refreshParent();
                    }); }}>
                  ...
                  <DropdownMenuItem className="text-red-600 focus:bg-red-50 focus:text-red-600" onClick={(e) => {
                    // Have to slightly delay delete trigger so dropdown menu closes properly
                    setTimeout(() => handleDeleteChild(e), 10);
                  }}>

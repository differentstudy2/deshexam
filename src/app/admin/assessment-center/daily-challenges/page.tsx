...
import { DailyChallenge } from '@/lib/assessment-types';
...
export default function DailyChallengesPage() {
    ...
    const [editData, setEditData] = useState<Partial<DailyChallenge>>({});
    ...
    const fetchChallenges = async () => {
        setLoading(true);
        try {
            const data = await getAssessments('dailyChallenges');
            setChallenges(data as DailyChallenge[]);
        } catch (error) {
            console.error(error);
            toast({ title: 'Error fetching daily challenges', variant: 'destructive' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await deleteAssessment('dailyChallenges', id);
        ...
        return (
            <div className="p-6 max-w-5xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold">{editData.id ? 'Edit Daily Challenge' : 'Create Daily Challenge'}</h1>
                </div>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    ...
                                    <Input value={editData.title || ''} onChange={e => setEditData({...editData, title: e.target.value})} placeholder="E.g., Challenge of the Day: General Science" />
                                </div>
                                <div>
                                    <label className="text-sm font-medium">Slug</label>
                                    <Input value={editData.slug || ''} onChange={e => setEditData({...editData, slug: e.target.value})} placeholder="daily-challenge-sci-1" />
                                </div>
                                <div>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    ...
                                    <Input type="date" value={editData.activeDate || ''} onChange={e => setEditData({...editData, activeDate: e.target.value})} />
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow><TableCell colSpan={5} className="text-center">No challenges found.</TableCell></TableRow>
                            ) : (
                                    ...
                                    <TableRow key={challenge.id}>
                                        ...
                                        <TableCell>
                                            <div className="flex gap-2">
                                                <Button variant="ghost" size="sm" onClick={() => handleEdit(challenge)}><Pencil className="h-4 w-4" /></Button>
                                                <Button variant="ghost" size="sm" className="text-red-500" onClick={() => handleDelete(challenge.id)}><Trash2 className="h-4 w-4" /></Button>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))

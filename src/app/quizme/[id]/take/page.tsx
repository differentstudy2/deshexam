...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
  ...
  return {
    title: `Take ${quiz.title} | DeshExam`,
  ...
  if (questions.length === 0) {
    return (
      <div className="container max-w-5xl mx-auto py-12 px-4 text-center">
        ...
        <Link href={`/quizme/${quiz.slug}`} className="text-purple-600 mt-4 inline-block font-medium">
          &larr; Go Back
        </Link>
      </div>
    );
    ...
    <div className="bg-slate-50 min-h-screen pb-12">
      <div className="bg-white border-b py-4 mb-8 sticky top-0 z-10 shadow-sm">
        <div className="container max-w-5xl mx-auto px-4 flex items-center justify-between">
          <Link href={`/quizme/${quiz.slug}`} className="text-slate-500 hover:text-slate-900 flex items-center font-medium">

...
export function AssessmentCard({ assessment, type, href }: AssessmentCardProps) {
  ...
  if (type === 'Practice') {
    typeColor = 'bg-[#00a651]/10 text-[#00a651] border-[#00a651]/20';
    Icon = FileText;
  } else if (type === 'Quiz') {
  ...
  // Difficulty Colors
  let diffColor = 'bg-slate-100 text-slate-700';
  if (assessment.difficulty === 'Easy') diffColor = 'bg-green-100 text-green-700';
  if (assessment.difficulty === 'Medium') diffColor = 'bg-yellow-100 text-yellow-700';
  if (assessment.difficulty === 'Hard') diffColor = 'bg-orange-100 text-orange-700';
  if (assessment.difficulty === 'Expert') diffColor = 'bg-red-100 text-red-700';

  return (
    <Card className="flex flex-col h-full overflow-hidden hover:shadow-lg transition-all duration-300 border-slate-200 hover:border-[#00a651]/50 group">
      ...
      <CardHeader className="pb-3 flex-none">
        ...
        <h3 className="font-bold text-lg leading-tight line-clamp-2 text-slate-800 group-hover:text-[#00a651] transition-colors">
          {assessment.title}
        </h3>
      </CardHeader>
      ...
      <CardFooter className="pt-0 flex-none">
        <Button asChild className="w-full bg-slate-900 hover:bg-[#00a651] text-white transition-colors group-hover:bg-[#00a651]">
          <Link href={href}>

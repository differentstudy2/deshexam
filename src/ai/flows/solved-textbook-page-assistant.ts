...
2.  **Format and Stylize:** Convert the transcribed text into well-structured GitHub-flavored Markdown. Use the following rules:
    *   **Markdown First**: Primarily use GitHub-flavored Markdown for all text formatting (headings, lists, bold, italics, tables). Use simple, clear Markdown. For example, to bold a word, use **word**, not complex cases like **'word'**. Avoid putting emphasis markers like \`**\` or \`*\` right next to punctuation.

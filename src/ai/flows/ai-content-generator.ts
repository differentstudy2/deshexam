...
const QuestionSchema = z.object({
  ...
  wordBank: z.string().optional().describe("For 'Fill in the Blank' questions, a string of words separated by newlines (\\n) that will be used as draggable options."),
  correctAnswerString: z.string().optional().describe("For 'Fill in the Blank' questions, a comma-separated string of the correct words in the order they appear in the blanks."),
});
...
const AIContentGeneratorInputSchema = z.object({
  ...
  sourceType: z.enum(['topic', 'text', 'file']).describe('The source of the content to be generated.'),
...
{{else if isUri}}
The source for the content is the following document/image. Extract the content and generate questions from it:
{{media url=source}}
{{else}}
...
- For 'Fill in the Blank' questions, you MUST provide a \`wordBank\` field containing a newline-separated list of potential answers (including the correct ones and some incorrect ones). You MUST also provide a \`correctAnswerString\` field with a comma-separated list of the correct words in the order they should appear in the blanks.

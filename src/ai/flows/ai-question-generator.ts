...
- For 'Multiple Choice' questions, provide exactly 4 options. For each option, provide the option text and a brief explanation stating why the option is correct or incorrect. For example: "This is correct because..." or "This is incorrect because...".
- For 'True/False' questions, you MUST provide an 'options' array with two items. The first item is for 'True' and the second is for 'False'. Each item MUST have a detailed explanation. For example: \`options: [{ "text": "True", "explanation": "This is incorrect because..." }, { "text": "False", "explanation": "This is correct because..." }]\`.
...
- A general explanation for the correct answer that summarizes the main concept. This field is mandatory for ALL question types. For "Multiple Choice", summarize the correct concept. For "True/False" questions, explain why the statement is true or false. For Fill in the Blank, explain the concept behind the answer. For Matching, provide a summary of the relationships.
  ...
  async (input) => {
    ...
    if (output && output.questions) {
      output.questions = output.questions.map(q => {
        ...
        // Ensure True/False questions have the standard options array if not provided
        if (q.type === 'True/False' && (!q.options || q.options.length !== 2)) {
          q.options = [{ text: 'True', explanation: '' }, { text: 'False', explanation: '' }];
        }
        return q;
      });
    }

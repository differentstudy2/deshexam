...
const treeNodeStart = content.indexOf('type TreeNodeProps = {');
const treeNodeEnd = content.indexOf('export default function ContentExplorer() {');
...
content = content.substring(0, treeNodeStart) + newHelpers + '\n' + content.substring(treeNodeEnd);

// Now replace the ContentExplorer render block (Lines 696-738)
const mainReturnStart = content.indexOf('  return (\n    <div className="space-y-6">');
const mainReturnEnd = content.indexOf('      {/* SEO Dialog */}');

const newRender = `  // Native UI State
  ...
  const handleSelectNode = async (node: any) => {
    ...
    // Update breadcrumbs (rough approximation, real one would trace path)
  ...
  // Re-map onSuccess to refreshMainPane
  const withRefresh = (onSuccess: () => void) => () => {
    onSuccess();
    refreshMainPane();
  };

  return (
    <div className="flex flex-col h-[calc(100vh-80px)] -m-4 sm:-m-8 bg-white dark:bg-slate-950">
      {/* Top Header Bar */}
                  ...
                  <TableBody>
                    {mainChildren.map((child, index) => (
                      <TableRow key={child.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 group cursor-pointer" onClick={() => { if (child.type !== 'section') handleSelectNode(child); }}>
                        ...
                        <TableCell className="capitalize text-slate-500">{child.type}</TableCell>
                        <TableCell className="text-slate-500">{child.author || '-'}</TableCell>
...
content = content.substring(0, mainReturnStart) + newRender + content.substring(mainReturnEnd);

fs.writeFileSync('f:/developer/deshexam/src/app/admin/guide-content/explorer/page.tsx', content);

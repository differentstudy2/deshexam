...
// 1. Remove TreeNode component completely
const treeNodeStart = code.indexOf('type TreeNodeProps');
const treeNodeEnd = code.indexOf('export default function ContentExplorer() {');
if (treeNodeStart !== -1 && treeNodeEnd !== -1) {
  code = code.substring(0, treeNodeStart) + code.substring(treeNodeEnd);
}

// 2. Change export
code = code.replace('export default function ContentExplorer() {', 'export function MobileExplorer({ className }: { className?: string }) {');

// 3. Replace state and fetchRoot
const stateStart = code.indexOf('const { toast } = useToast();');
const effectEnd = code.indexOf('const handleMoveBoard = async');
...
`;
code = code.substring(0, stateStart) + newState + code.substring(effectEnd);

// 4. Replace handleMoveBoard
code = code.replace(/const handleMoveBoard = async \([\s\S]*?\} catch \(e\) \{\s*console\.error\("Failed to reorder boards", e\);\s*\}\s*\};/, `
...
// 5. Replace main return block
const returnStart = code.indexOf('return (');
const returnEnd = code.indexOf('{/* SEO Dialog */}');
if (returnStart !== -1 && returnEnd !== -1) {
  const mobileUI = `return (
    <div className={\`flex flex-col h-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden \${className || ''}\`}>
      ...
      `;
  code = code.substring(0, returnStart) + mobileUI + code.substring(returnEnd);
}

fs.writeFileSync(path, code);
console.log('MobileExplorer modified!');

...
function addFeatures() {
    ...
    // The current related FAQs render starts with:
    ...
    // We want to change the div to a Link, and add view count next to category.
    content = content.replace(
        /<div key=\{idx\} className="bg-white border border-slate-200 rounded-lg p-5 hover:border-slate-300 transition-colors cursor-pointer flex flex-col h-full">/g,
        '<Link href={`/faq/${faq.seo?.slug || faq.id}`} key={idx} className="bg-white border border-slate-200 rounded-lg p-5 hover:border-slate-300 transition-colors cursor-pointer flex flex-col h-full">'
    );
    // At the end of the related faq card:
    content = content.replace(
        /<\/div>\s*<\/div>\s*\)\)/g, // Wait, there's a div closing the card. Since I used Link, I have to change the closing tag.
        // Actually it's better to just do string replace on the whole map block to be extremely safe.
        "" // we will do it differently
    );
    // Let's replace the whole related faq block
...
addFeatures();
console.log("Added features properly");
